...
#[doc = " \"Normalized hit distance\" = saturate( \"hit distance\" / f ), where:\n f = ( A + viewZ * B ) * lerp( 1.0, C, exp2( D * roughness ^ 2 ) ), see \"NRD.hlsl/REBLUR_FrontEnd_GetNormHitDist\""]
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct nrd_HitDistanceParameters {
    #[doc = " (units > 0) - constant value"]
    pub A: f32,
    #[doc = " (> 0) - viewZ based linear scale (1 m - 10 cm, 10 m - 1 m, 100 m - 10 m)"]
    pub B: f32,
    #[doc = " (>= 1) - roughness based scale, use values > 1 to get bigger hit distance for low roughness"]
    pub C: f32,
    #[doc = " (<= 0) - absolute value should be big enough to collapse \"exp2( D * roughness ^ 2 )\" to \"~0\" for roughness = 1"]
    pub D: f32,
}
#[allow(clippy::unnecessary_operation, clippy::identity_op)]
const _: () = {
...
pub struct nrd_ReblurAntilagSettings {
    #[doc = " [1; 5] - delta is reduced by local variance multiplied by this value"]
    pub luminanceSigmaScale: f32,
    #[doc = " [1; 5] - antilag sensitivity (smaller values increase sensitivity)"]
...
pub struct nrd_ReblurSettings {
    ...
    #[doc = " [0; REBLUR_MAX_HISTORY_FRAME_NUM] - maximum number of linearly accumulated frames"]
    pub maxAccumulatedFrameNum: u32,
    #[doc = " [0; maxAccumulatedFrameNum) - maximum number of linearly accumulated frames for fast history\n Values \">= maxAccumulatedFrameNum\" disable fast history\n Usually 5x times shorter than the main history"]
    pub maxFastAccumulatedFrameNum: u32,
    #[doc = " [0; maxAccumulatedFrameNum] - maximum number of linearly accumulated frames for stabilized radiance\n \"0\" disables the stabilization pass\n Values \">= maxAccumulatedFrameNum\"  get clamped to \"maxAccumulatedFrameNum\""]
    pub maxStabilizedFrameNum: u32,
    #[doc = " [0; 3] - number of reconstructed frames after history reset (less than \"maxFastAccumulatedFrameNum\")"]
    pub historyFixFrameNum: u32,
    #[doc = " (> 0) - base stride between pixels in 5x5 history reconstruction kernel (gets reduced over time)"]
    pub historyFixBasePixelStride: u32,
    #[doc = " (pixels) - pre-accumulation spatial reuse pass blur radius (0 = disabled, must be used in case of badly defined signals and probabilistic sampling)"]
    pub diffusePrepassBlurRadius: f32,
    pub specularPrepassBlurRadius: f32,
    #[doc = " (0; 0.2] - bigger values reduce sensitivity to shadows in spatial passes, smaller values are recommended for signals with relatively clean hit distance (like RTXDI/RESTIR)"]
    pub minHitDistanceWeight: f32,
    #[doc = " (pixels) - min denoising radius (for converged state)"]
    pub minBlurRadius: f32,
    #[doc = " (pixels) - base (max) denoising radius (gets reduced over time)"]
    pub maxBlurRadius: f32,
    #[doc = " (normalized %) - base fraction of diffuse or specular lobe angle used to drive normal based rejection"]
    pub lobeAngleFraction: f32,
    #[doc = " (normalized %) - base fraction of center roughness used to drive roughness based rejection"]
    pub roughnessFraction: f32,
    #[doc = " [0; 1] - if roughness < this, temporal accumulation becomes responsive and driven by roughness (useful for animated water)"]
    pub responsiveAccumulationRoughnessThreshold: f32,
    #[doc = " (normalized %) - represents maximum allowed deviation from the local tangent plane"]
    pub planeDistanceSensitivity: f32,
    #[doc = " \"IN_MV = lerp(IN_MV, specularMotion, smoothstep(this[0], this[1], specularProbability))\""]
    pub specularProbabilityThresholdsForMvModification: [f32; 2usize],
    #[doc = " [1; 3] - undesired sporadic outliers suppression to keep output stable (smaller values maximize suppression in exchange of bias)"]
    pub fireflySuppressorMinRelativeScale: f32,
    #[doc = " (Optional) material ID comparison: max(m0, minMaterial) == max(m1, minMaterial) (requires \"NormalEncoding::R10_G10_B10_A2_UNORM\")"]
    pub minMaterialForDiffuse: f32,
    pub minMaterialForSpecular: f32,
    #[doc = " If not OFF and used for DIFFUSE_SPECULAR, defines diffuse orientation, specular orientation is the opposite. Used only if \"NRD_SUPPORTS_CHECKERBOARD = 1\""]
    pub checkerboardMode: nrd_CheckerboardMode,
    #[doc = " Must be used only in case of probabilistic sampling (not checkerboarding), when a pixel can be skipped and have \"0\" (invalid) hit distance"]
    pub hitDistanceReconstructionMode: nrd_HitDistanceReconstructionMode,
    #[doc = " Adds bias in case of badly defined signals, but tries to fight with fireflies"]
    pub enableAntiFirefly: bool,
    #[doc = " In rare cases, when bright samples are so sparse that any other bright neighbor can't\n be reached, pre-pass transforms a standalone bright pixel into a standalone bright blob,\n worsening the situation. Despite that it's a problem of sampling, the denoiser needs to\n handle it somehow on its side too. Diffuse pre-pass can be just disabled, but for specular\n it's still needed to find optimal hit distance for tracking. This boolean allow to use\n specular pre-pass for tracking purposes only (use with care)"]
